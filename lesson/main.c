...
// Макрос для измерения и печати времени выполнения функций
#define MEASURE_AND_PRINT(fn_before, fn_after, num) do { \
    printf("Пример %03d:\n", num); \
    long start = get_microseconds(); \
    fn_before(); \
    long end = get_microseconds(); \
    printf("Время до рефакторинга: %ld мкс\n", end - start); \
    start = get_microseconds(); \
    fn_after(); \
    end = get_microseconds(); \
    printf("Время после рефакторинга: %ld мкс\n\n", end - start); \
} while(0)


// ████████████████████████████████████████████████
// ✨ Список активных примеров (добавьте сюда номера)
...
// Определение функций через X-Macro
#define X(n) \
